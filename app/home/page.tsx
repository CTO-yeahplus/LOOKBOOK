...
function HomeContent() {
  ...
  useEffect(() => {
    if (Capacitor.isNativePlatform()) {
      // 🌟 카카오톡/인스타 등에서 링크를 눌러 앱이 켜졌을 때 URL을 낚아채는 안테나
      CapacitorApp.addListener('appUrlOpen', data => {
        console.log('App opened with URL:', data.url);
        // 예: https://auraootd.com/home?item_id=123
        const urlObject = new URL(data.url);
        const itemId = urlObject.searchParams.get('item_id');
        
        if (itemId) {
          // Next.js 라우터를 사용해 해당 상세 페이지로 쏴줍니다!
          window.location.href = `/home?item_id=${itemId}`;
        }
      });
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      // 🌟 바로 다음 장(+1) 딱 한 장만 미리 불러와서 메모리를 아낍니다.
      const targetIndex = (aura.currentIndex + 1) % aura.fashionItems.length;
      const targetItem = aura.fashionItems[targetIndex];
      
      if (targetItem?.imageUrl) {
        const img = new Image();
        img.crossOrigin = "anonymous"; 
        img.decoding = "async"; // 브라우저가 멈추지 않게 비동기 해독 지시
    ...
    return (
      <main className="relative min-h-screen bg-[#050505] flex flex-col items-center justify-center selection:bg-[#ff3b30] selection:text-white">
        ...
        <LoginModal 
          ...
          // 🌟 [수술 완료] 애플 로그인만 네이티브 엔진으로 가로채는 우회로(Bypass) 설계
          onSignIn={async (provider) => {
            // [수술 부위: onSignIn 함수 내부]
            if (provider === 'apple' && Capacitor.isNativePlatform()) {
              try {
                ...
                const response = result.response as AppleResponse;
                const autoGeneratedNonce = response.nonce;
                ...
                if (response.identityToken) {
                  ...
                  alert('성공! 여기서 튕기는지 확인하세요.');

                  // ✅ 팝업 대신 조용히 모달을 닫고 메인으로 이동합니다.
            ...
            // 3. 애플이 아닌 구글, 깃허브 등은 기존 로직 그대로 탑승!
            else {
  ...
  return (
    ...
    >
      ...
      <motion.div 
        ...
        initial={{ opacity: 0 }} 
        animate={{ opacity: 1 }} // 🌟 이미지가 아니므로 투명도 조절 불필요
        exit={{ opacity: 0 }} 
        transition={{ duration: 0.3 }}
        className="absolute inset-0 z-0 pointer-events-none transform-gpu"
        // 🌟 [핵심] 이미지 태그 대신, 현재 옷차림의 주요 색상을 배경색으로 주입합니다!
        style={{ 
          backgroundColor: currentItem.colors?.[0] || "#111111" // 색상 데이터가 없다면 기본 어둠
        }}
      >
        {/* 약간의 무드를 위해 그라데이션 오버레이만 추가 */}
...
export default function Home() {
  return (
    // 클라이언트에서 URL 파라미터를 읽어올 때까지 잠깐 보여줄 빈 화면(또는 로더)
